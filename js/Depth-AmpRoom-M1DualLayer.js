...
const audioFiles8Close = ['Depth-AmpRoom/Close/1', 'Depth-AmpRoom/Close/2', 'Depth-AmpRoom/Close/3', 'Depth-AmpRoom/Close/4', 'Depth-AmpRoom/Close/5', 'Depth-AmpRoom/Close/6', 'Depth-AmpRoom/Close/7', 'Depth-AmpRoom/Close/8'];
...
const PlayerClose = new Mach1SoundPlayer(getAudioFiles(audioFiles8Close));
const PlayerFar = new Mach1SoundPlayer(getAudioFiles(audioFiles8Far));
...
async function trackerMain() {
  ...
  const waitingSounds = () => new Promise((resolve) => {
    const timer = setInterval(() => {
      progress.change(PlayerClose.progress); // update loading info
      if (PlayerClose.isReady() && PlayerFar.isReady()) {
        clearInterval(timer);
        resolve();
      }
    }, 500);
