...
foaRenderer.initialize().then(function() {
  ...
  foaRenderer.output.connect(ambiAudioContext.destination);
});
...
async function trackerMain() {
  ...
  const waitingSounds = () => new Promise((resolve) => {
    const timer = setInterval(() => {
      PlayerAmbisonic.load();
      progress.change(PlayerM1.progress); // update loading info
      if (PlayerM1.isReady()) {
        clearInterval(timer);
        resolve();
      }
...
function Play() {
    PlayerM1.play();
    ambiAudioContext.resume();
    PlayerAmbisonic.play();
}
